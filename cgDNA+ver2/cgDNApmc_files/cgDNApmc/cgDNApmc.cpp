...
int handle_command_line(int argc, char *argv[], vector<int> &chosen_t,
    ...
    // Options for the way numebr of configurations is provided
    const string ACC_CONFIGS = "a";
    const string GEN_CONFIGS = "g";
    ...
    result = get_requested_expectations(string(argv[0]), chosen_t, chosen_s,flory_chosen);
    if (result != 0) {
    ...
    // Input sequence file
    inut_seq_file = argv[2];
    // Parameter set file
    inut_paramset = argv[3];
    
    if (string(argv[4]) == ACC_CONFIGS) {
        // Number of configs accepted
        num_configs_acc = atoi(argv[5]);
    } else if (string(argv[4]) == GEN_CONFIGS) {
        // Number of configs generated
        num_configs_gen = atoi(argv[5]);
    }
    
    // Number of basepair to drop for both ends
    num_dropped_bp = atoi(argv[6]);
    
    // Use Jacobian
    if (string(argv[7]) == JAC_YES) {
            use_jac = true;
        } else if (string(argv[7]) == JAC_NO) {
            use_jac = false;
        } else {
            // Print to standard error output
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    // Type of coarse-grain model. Default is 2 which is cgDNAp
    int Model = 2;
    ...
    log_stream.open((label + ".log").c_str());
    ...
    if (result != 0) {
        cout << "Problem in loading the parameter set" << endl;
        return ;
    }
    ...
    if (result != 0) {
        cout << "Problem in building the Gaussian" << endl;
        return ;
    }
    ...
    if (result != 0) {
        cout << "Problem in computing the Cholesky decomposition" << endl;
        return ;
    }
    ...
    if (!chosen_t.empty() || !chosen_s.empty()) {
        ...
        reconstructBPVecD(oligo_w_hat, r_bp, R_bp, Model);
    ...
    ++trial) {
        ...
        if (use_jac) {
            jac = compute_jacobian(drawn_conf);
        ...
        if (accept) {
            ...
            reconstructBPVecD(drawn_conf, r_bp, R_bp, Model);
