...
int load_cgdna_parameter_set(string filename,
	...
	for (int i = 0; i < 36; ++i) {
		...
		read_block_and_c(param_file, 42, dimer_stiff[ind], dimer_shape[ind]);
	...
	for (int i = 0; i < 16; ++i) {
		...
		read_block_and_c(param_file, 42, end1_stiff[ind], end1_shape[ind]);
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        read_block_and_c(param_file, 42, end2_stiff[ind], end2_shape[ind]);
        indRead[i] = true;
    }
...
int build_cgdna_stiff_and_sigma(string seq,
	...
	oligo_stiff.resize(num_pars * 42, 0.0);
	...
	for (int i = 1; i < num_bp - 2; ++i) {
		...
		//stiffness
		for (int j = 0; j < 42*42; ++j) {
			oligo_stiff[42*(18+24*(i-1)) + j] += dimer_stiff[dimer_param_ind][j];
		}


		//weighted shape vector c = K*\hat{w}
		for (int j = 0; j < 42; ++j) {
			oligo_sigma[18 + 24 * (i-1) + j] += dimer_shapes[dimer_param_ind][j];
		}
    ...
    // Then the back end
        ...
        //stiffness
        for (int j = 0; j < 42 * 42; ++j) {
            oligo_stiff[42*(12+24*(num_bp-3)) + j ] += end2_stiff[dimer_param_ind][j];
        }
    
        //weighted shape vector c = K*\hat{w}
        for (int j = 0; j < 42; ++j) {
        oligo_sigma[12 + 24 * (num_bp-3) + j] += end2_shapes[dimer_param_ind][j];
        }
...
int compute_cgdna_chol_stiff_and_w_hat(std::vector<double> &oligo_stiff,
	...
	if (num_pars % 24 != 6 && oligo_stiff.size() == num_pars * 42) {
		return 1;
	}

	// Compute Cholesky decomposition
	result = compute_cholesky_band(41, oligo_stiff);
	if (result != 0) {
		cout << "4"<< endl ; 
		return result;
	}
	...
	result = solve_band_trian('N', 41, oligo_stiff, oligo_shape);
	if (result != 0) {
		return 2 * result;
	}
	result = solve_band_trian('T', 41, oligo_stiff, oligo_shape);
...
double eval_cgdna_en(const std::vector<double> &oligo_stiff,
	...
	if (oligo_stiff.size() != num_coords * 42) {
		log_stream
	...
	//           = L^T * config
	mult_band_trian_mat_vec('T', 41, oligo_stiff, config);
