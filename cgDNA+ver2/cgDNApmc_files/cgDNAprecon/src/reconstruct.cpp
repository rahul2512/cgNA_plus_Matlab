...
void nextBasePair(int i, const fpType *params, //
		Vector3<fpType> &q, Quaternion<fpType> &g, int baseModel) {
	Quaternion<fpType> L;
	Vector3<fpType> zeta;
	Quaternion<fpType> h;
	...
	int firstIndex = 0;
	int secondIndex = 0;
	/* Different coarse-grained model allowed :
	   case = 2 : rigid base-phosphate model (cgDNAp) 
	   case = 1 : rigid base model (cgDNA) 
	   case = 0 : rigid basepair model 	
	*/
    switch (baseModel) {
		case 2:	
			firstIndex = 24 * i + 12;
...
void nextBasePairM(int i, const fpType *params, //
		Vector3<fpType> &q, Matrix3<fpType> &g, int baseModel) {
